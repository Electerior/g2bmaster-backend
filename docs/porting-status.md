...
| 첨부 파싱 (HWPX·HWP·PDF·XLSX·ZIP) | ✅ 완료 |
| 첨부 본문 색인 + 파일 내 키워드 검색 | ✅ 완료 — `docs/attachment-text-index.md` |
| 엑셀 내보내기 | ❌ 미착수 |
...
아래 판단은 **틀렸다**. hwplib·hwpxlib·PDFBox·POI 로 `DocumentTextExtractor` 가 이식됐고,
그 위에 첨부 본문 색인(`bid_notice_document`)과 파일 내 키워드 검색이 올라갔다 —
설계와 실측 근거는 `docs/attachment-text-index.md`. 원문은 판단 이력으로 남긴다.
