...
| 메서드 | 경로 | 인증 | 설명 |
|---|---|---|---|
| GET | `/api/search/notices` | — | 검색. **공고 텍스트 + 첨부 본문**. `{items,totalCount,pageNo,numOfRows,meta}` |
| GET | `/api/search/notices/facets` | — | 같은 조건·**같은 후보 집합**의 분류·업종·지역·상태별 건수 + `total` |
| GET | `/api/search/notices/text` | — | 검색. **공고 텍스트만**(첨부 제외). 파라미터·봉투 동일 |
...
### 4.0 기본 검색이 첨부 본문까지 보는 이유 — 그리고 `/text` 를 따로 뺀 이유

사용자가 '리튬인산철'을 치는 이유는 **그 낱말이 든 공고**를 찾으려는 것이지, *공고 제목에*
그 낱말이 든 공고를 찾으려는 것이 아니다. 사양은 대부분 규격서에만 적혀 있으므로, 첨부를
보지 않는 검색은 "그런 공고가 없다"고 거짓말을 하게 된다. 그래서 **기본이 첨부 포함**이다.

그렇다고 첨부를 안 보는 검색이 없어도 되는 것은 아니다. 둘은 비용과 성격이 다르다.

| | 질의 | 실측(§4.5) | 결과의 성격 |
|---|---|---|---|
| `/api/search/notices` | UNION 두 갈래 | 19.9ms | 첨부 색인 진행도에 따라 결과가 늘어난다 |
| `/api/search/notices/text` | 한 갈래 | 5.3ms | 색인 상태와 무관하게 일정하다 |

파라미터 하나(`scope=…`)로 가르지 않고 **경로를 나눈** 이유가 여기 있다. 자동완성처럼
응답 시간이 곧 기능인 경로와 "제대로 찾아야 하는" 경로는 캐시·타임아웃 정책이 달라야 하는데,
같은 URL 이면 어느 화면이 무엇을 본 검색인지 나중에 아무도 설명하지 못한다.

> ⚠ `/text` 는 `/{id}` 와 같은 자리를 겨룬다. 스프링이 리터럴을 먼저 골라 동작은 정확하지만,
> 공고번호가 정확히 `text` 인 건은 상세로 갈 수 없다. 공고번호는 숫자·하이픈뿐이라 실제로는
> 생기지 않는 충돌이라 감수했다.

**비상 손잡이**: `g2b.search.attachment-body=false`(`SEARCH_ATTACHMENT_BODY`) 면 기본 경로가
`/text` 와 같아진다. 색인이 커져 검색이 느려진 날 배포 없이 되돌리기 위한 것이고, 응답의
`meta.attachmentSearch.scope` 가 `false` 로 내려가 화면이 그 사실을 알 수 있다.
...
모든 정렬 뒤에 `n.id ASC` 가 타이브레이커로 붙는다. 같은 날 올라온 공고가 수백 건이라,
없으면 MySQL 이 페이지마다 다른 순서를 줘서 2페이지에 1페이지 공고가 또 나오거나 건너뛴다.

**방향이 `ASC` 인 것에 이유가 있다.** InnoDB 보조 인덱스는 키가 같은 구간 안에서 PK 를
오름차순으로 담는다. `created_date DESC, id ASC` 는 `ix_bid_notice_category_created` 의 저장
순서와 일치해 정렬 비용이 0 이지만, `id DESC` 로 두면 같은 인덱스를 쓰고도 전체를 다시
정렬한다. 실측(17,477행, `category='입찰'`)에서 `DESC` 는 8,738행에 `Using filesort` 가 붙고
`ASC` 는 붙지 않는다. 타이브레이커의 목적은 순서를 고정하는 것이지 특정 방향이 아니다.
...
- `attachmentIndexed` — 이 공고의 첨부 본문 색인이 끝났는가. `false` 는 "첨부에 그 낱말이
  없다"가 아니라 **"첨부로는 판단할 수 없다"**는 뜻이다(첨부가 없는 공고도 `false`)

### 4.5 첨부 본문까지 보는 검색의 SQL

두 브랜치를 **`UNION ALL` 로 가르고** 그룹으로 접는다. 구현은
`BidNoticeIndexRepository.candidateSql`, 모양을 고정하는 테스트는 `AttachmentSearchSqlTest`.

```
공고 브랜치 : MATCH(notice_name, notice_body)  + 필터        → relevance = 매치 점수
첨부 브랜치 : MATCH(bid_notice_document.body_text) + 필터     → relevance = 0
            └ UNION ALL → GROUP BY (id, source) → notice_hit / doc_hit
제외        : 제외 낱말이 첨부에 있는 공고 집합을 한 번 뽑아 LEFT JOIN … IS NULL
```

**`OR` 로 합치면 안 된다.** 읽기에는 `MATCH(공고) OR EXISTS(MATCH(첨부))` 가 자연스럽지만,
그러면 두 FULLTEXT 인덱스 중 **어느 쪽도 구동 경로가 되지 못해** `bid_notice` 를 전수 훑으며
행마다 상관 서브쿼리를 돌린다.

실측 (2026-08-12, `bid_notice` 45,736행 · `bid_notice_document` done 3,704행,
검색어 '서버' → 공고 571건 · 첨부 502행, 마감 전 + 입찰문서 필터, 관련도순 20건):

| 형태 | 페이지 질의 | count |
|---|---|---|
| 공고 텍스트만 (기존 2단 질의) | **5.3ms** | 7.5ms |
| UNION ALL (공고 ∪ 첨부) | **19.9ms** | 21.6ms |
| UNION + 첨부 제외 반조인 | 21.5ms | 24.2ms |
| 제외를 상관 `NOT EXISTS` 로 | — | 34.6ms |
| `MATCH(공고) OR EXISTS(MATCH(첨부))` | **424ms** | — |
| 첨부에 한 글자 `LIKE '%컵%'` | — | **669ms** |

엔드포인트 왕복(count + 직렬화 포함)은 기본 33~39ms, `/text` 10~11ms 였다.

세 가지 결정이 이 표에서 나온다.

1. **관련도는 공고 텍스트 점수만 쓴다.** 첨부에서만 걸린 행은 0점이라 관련도순 뒤로 간다 —
   제목·본문 매치가 더 강한 신호이기도 하고, 두 점수를 합산하면 그룹 뒤에 다시 정렬해야 한다.
2. **제외는 집합을 한 번 뽑아 반조인한다.** 제외 낱말은 후보마다 달라지지 않는데 상관
   서브쿼리는 그것을 행마다 되풀이한다(34.6 → 24.2ms).
3. **한 글자 낱말은 첨부 범위에서 뺀다.** ngram 토큰이 2라 `MATCH` 가 통째로 버리고, 대안인
   `LIKE '%x%'` 는 done 3,704행만으로 이미 669ms 다. 뺀 낱말은 `meta.attachmentSearch.skippedTerms`
   로 응답에 싣는다 — 조용히 무시하면 사용자가 못 알아챈다. 공고 텍스트 쪽에서는 지금처럼
   `LIKE` 로 계속 걸린다(`notice_body` 는 훨씬 짧다).

**첨부의 AND 는 파일 단위다.** `+"서버" +"스토리지"` 는 *한 파일 안에* 둘 다 있어야 걸린다.
규격서 하나가 곧 그 공고의 사양이라 실무 의미가 있고, 공고 단위로 접으려면 낱말마다 서브쿼리를
따로 걸어야 해서 비용이 낱말 수에 비례한다.

### 4.6 커버리지를 응답에 싣는 이유

첨부 검색은 **색인된 만큼만** 동작한다. 2026-08-12 기준 45,736건 중 1,225건(2.7%)이다.
그 사실을 응답에 적지 않으면 화면에서 "그 낱말이 없는 공고"와 "아직 읽지 못한 공고"가
똑같이 보이고, 규격서에 답이 있는 공고를 사용자가 조용히 놓친다
(`bid_notice_document.needs_ocr` 을 남긴 것과 같은 이유다).

```json
"meta": { "attachmentSearch": {
  "scope": true,            // 이 엔드포인트가 첨부를 보기로 돼 있는가
  "applied": true,          // 이번 질의가 실제로 첨부를 뒤졌는가(검색어가 없으면 false)
  "excludeApplied": false,  // 제외 낱말을 첨부까지 적용했는가
  "skippedTerms": ["차"],   // 한 글자라 첨부 범위에서 뺀 낱말
  "totalNotices": 45736, "indexedNotices": 1225
}}
```

커버리지 두 칸은 60초 캐시다. 값이 아니라 **비용** 때문이다 — 집계가 실측 6.3ms 라 20ms 짜리
검색에 매번 붙이면 30% 를 세는 데 쓴다. 추출 워커가 30분 주기라 1분 낡은 값은 문제가 없다.
...
- **`notice_body` 는 공고 원문이 아니다**(§2.3). 첨부 본문은 별도 계통이 담는다 —
  `bid_notice_document` + `docs/attachment-text-index.md`.
