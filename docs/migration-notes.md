...
새 마이그레이션을 추가하는 방법(`./tools/new-migration.sh`)과 번호 규칙은
[README `마이그레이션 추가`](../README.md#마이그레이션-추가) 에 있다. 요약하면 **V1~V14 는
순번, 그 뒤로는 전부 14자리 타임스탬프**다 — 브랜치별로 순번을 집다가 V9/V10 과 V11~V14 가
서로를 모른 채 만들어진 사고 이후로 바꿨다.
